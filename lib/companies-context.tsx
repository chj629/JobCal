...
export function CompaniesProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      // 동일한 이유) — retryable이면 실제로 로그아웃된 게 아니므로 currentUserId/userId/
      // companies를 전혀 건드리지 않고 loading만 정리한다. 화면에 이미 있던 정상 데이터를
      // 일시적 오류로 비우지 않기 위함이며, 인증 성공으로 간주하는 것도 아니다 — 다음
      // 정상 호출(재마운트/실제 auth 이벤트)에서 다시 검증된다.
      if (!user && getUserError && isAuthRetryableFetchError(getUserError)) {
        if (process.env.NODE_ENV === "development") {
          console.warn(
            "[auth] getUser() 확인이 일시적으로 실패해(retryable) 기존 companies를 유지합니다.",
