...
export function CompanyCredentialsProvider({ children }: { children: ReactNode }) {
  ...
  // company_credentials만 다시 읽어와 로컬 state를 교체한다. loading/userId는 건드리지
  // 않는다 — 호출부(load 또는 backgroundRefresh)가 각자의 의미에 맞게 관리한다. 실패해도
  // 기존 credentials는 그대로 두고(이미 보고 있던 화면이 사라지면 안 된다) error만 알린다.
  ...
  async function load() {
    ...
    // userId/credentials를 전혀 건드리지 않고 loading만 정리한다. 화면에 이미 있던
    // 정상 데이터를 일시적 오류로 비우지 않기 위함이며, 인증 성공으로 간주하는 것도
    // 아니다 — 다음 정상 호출에서 다시 검증된다.
    if (!user && getUserError && isAuthRetryableFetchError(getUserError)) {
      if (process.env.NODE_ENV === "development") {
        console.warn(
          "[auth] getUser() 확인이 일시적으로 실패해(retryable) 기존 credentials를 유지합니다.",
  ...
  // 이미 화면에 credentials가 있는 상태에서의 조용한 재조회. 탭 복귀 시 supabase-js가
  ...
  useEffect(() => {
    ...
    const {
      ...
      // 실제 로그인/로그아웃/다른 계정으로 전환된 경우 — 이전 사용자의 credentials가
      // 화면에 잠깐이라도 남아있으면 안 되므로 즉시 비우고 다시 불러온다.
      setCredentials([]);
      load();
    });
