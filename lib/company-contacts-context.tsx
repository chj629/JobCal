...
export function CompanyContactsProvider({ children }: { children: ReactNode }) {
  ...
  // company_contacts만 다시 읽어와 로컬 state를 교체한다. loading/userId는 건드리지
  // 않는다 — 호출부(load 또는 backgroundRefresh)가 각자의 의미에 맞게 관리한다. 실패해도
  // 기존 contacts는 그대로 두고(이미 보고 있던 화면이 사라지면 안 된다) error만 알린다.
  ...
  async function load() {
    ...
    // userId/contacts를 전혀 건드리지 않고 loading만 정리한다. 화면에 이미 있던 정상
    ...
    if (!user && getUserError && isAuthRetryableFetchError(getUserError)) {
      if (process.env.NODE_ENV === "development") {
        console.warn(
          "[auth] getUser() 확인이 일시적으로 실패해(retryable) 기존 contacts를 유지합니다.",
  ...
  // 이미 화면에 contacts가 있는 상태에서의 조용한 재조회. 탭 복귀 시 supabase-js가
  ...
  useEffect(() => {
    ...
    const {
      ...
      // 실제 로그인/로그아웃/다른 계정으로 전환된 경우 — 이전 사용자의 contacts가 화면에
