...
// app/pricing/page.tsx(ja)와 app/ko/pricing/page.tsx(ko)가 공유하는 실제 페이지 본문 —
...
const POLICY_LINK_KEYS = [
  { jaPath: "/terms", labelKey: "termsLinkText" },
  { jaPath: "/privacy", labelKey: "privacyLinkText" },
  { jaPath: "/refund-policy", labelKey: "refundPolicyLinkText" },
  { jaPath: "/tokushoho", labelKey: "tokushohoLinkText" },
] as const;
...
function PricingPageInner() {
  ...
  const pathname = usePathname(); // "/pricing" 또는 "/ko/pricing" — 로그인 후 복귀 경로 계산에 쓴다.
      ...
      <main className="mx-auto max-w-5xl px-6 pb-24 pt-32 md:px-12">
        ...
        <div className="mx-auto mt-20 max-w-2xl border-t border-neutral-100 pt-10 text-center">
          ...
          <nav className="flex flex-wrap justify-center gap-x-8 gap-y-2 text-[13px]">
            {POLICY_LINK_KEYS.map(({ jaPath, labelKey }) => (
              <Link
                key={jaPath}
                href={toPublicPageHref(locale, jaPath)}
