...
// app/signup/page.tsx(ja)와 app/ko/signup/page.tsx(ko)가 공유하는 실제 페이지 본문 — 두
// 라우트 모두 이 컴포넌트를 그대로 렌더링하고, 감싸는 LocaleProvider(locked)의
// initialLocale만 다르다. 본문을 언어별로 복제하지 않는다. Supabase 로직(Google
// OAuth/emailRedirectTo 등)은 이번 단계에서 손대지 않았다 — /login·/terms·/privacy로
// 가는 내부 링크만 현재 locale에 맞춰 /ko/*로 이동하도록 고쳤다.
export default function SignupPageContent() {
  ...
  // 이미 계정이 있어 "ログイン" 링크로 로그인으로 넘어가는 사용자도 결제 의도를 잃지
  // 않도록, 지금 이 회원가입 URL의 next/checkout을 하나의 next 값으로 합쳐 그대로 넘긴다.
  // 의도가 없거나 안전하지 않으면 기존과 동일하게 그냥 현재 locale의 로그인 URL로 남는다.
  const [loginHref, setLoginHref] = useState(() => toPublicPageHref(locale, "/login"));
  ...
  useEffect(() => {
    ...
    if (checkoutNext) {
      setLoginHref(`${toPublicPageHref(locale, "/login")}?next=${encodeURIComponent(checkoutNext)}`);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // 기존 로그인 페이지(components/auth/LoginPageContent.tsx)의 handleGoogleLogin과 동일한
  // Supabase OAuth 로직. 회원가입/로그인 모두 같은 Google 계정 흐름을 타므로 로직은 그대로,
  ...
  async function handleGoogleSignup() {
    ...
    // /login(components/auth/LoginPageContent.tsx)의 handleGoogleLogin과 동일한 이유로
    // locale도 함께 실어 보낸다 — OAuth 실패/취소 시 되돌아갈 로그인 페이지를 고르는 데만
    // 쓰인다("ko"만 한국어, 그 외 전부 일본어로 취급하는 2택 enum).
    const params = new URLSearchParams(window.location.search);
    const checkoutNext = buildCheckoutNext(params.get("next"), params.get("checkout"));
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    // 검증을 다시 거친다(아래 /auth/confirm, /auth/confirmed 참고). locale도 함께 실어
    // 보낸다 — app/auth/confirm/route.ts가 이 값(정확히 "ja"|"ko" 둘 중 하나로만 검증,
    // 임의 경로가 아니라 이메일 인증 완료 후 보여줄 고정된 두 페이지 중 하나를 고르는
    // 용도라 open redirect 여지가 없다) 하나만 보고 /auth/confirmed 또는
    // /ko/auth/confirmed 중 지금 이 회원가입이 시작된 언어와 같은 쪽으로 보낸다.
    const params = new URLSearchParams(window.location.search);
    const checkoutNext = buildCheckoutNext(params.get("next"), params.get("checkout"));
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      options: {
        ...
        data: {
          ...
          // 가입 시 선택된 언어를 로그인 후 (app) 내부 LocaleProvider가 그대로 읽어갈 수
          // 있도록 user_metadata에도 저장한다(lib/locale-context.tsx의 기존 동기화 로직은
          // 건드리지 않음 — 그 로직이 이미 user_metadata.language를 읽고 있어서, 가입
          // 시점에 값을 채워 넣기만 하면 자연스럽게 연결된다).
          language: locale,
        },
      },
    });
              ...
              <div className="flex flex-col space-y-8">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <p className="mt-3 text-center text-[12px] leading-[1.6] text-neutral-500">
                    ...
                    <Link href={toPublicPageHref(locale, "/terms")} className="underline hover:text-neutral-900">
                      {t("auth.signup.agreementTermsLinkText")}
                    </Link>
                    {t("auth.signup.agreementMiddle")}
                    <Link href={toPublicPageHref(locale, "/privacy")} className="underline hover:text-neutral-900">
