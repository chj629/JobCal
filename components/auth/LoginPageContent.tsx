...
// app/login/page.tsx(ja)와 app/ko/login/page.tsx(ko)가 공유하는 실제 페이지 본문 — 두
// 라우트 모두 이 컴포넌트를 그대로 렌더링하고, 감싸는 LocaleProvider(locked)의
// initialLocale만 다르다. 본문을 언어별로 복제하지 않는다. Supabase 로직(Google
// OAuth/emailRedirectTo 등)은 이번 단계에서 손대지 않았다 — /forgot-password·/signup으로
...
export default function LoginPageContent() {
  ...
  // 계정이 없어 "新規登録" 링크로 회원가입으로 넘어가는 사용자도 결제 의도를 잃지 않도록,
  // 지금 이 로그인 URL의 next/checkout을 하나의 next 값으로 합쳐 그대로 넘긴다. 의도가
  // 없거나 안전하지 않으면 기존과 동일하게 그냥 현재 locale의 회원가입 URL로 남는다.
  const [signupHref, setSignupHref] = useState(() => toPublicPageHref(locale, "/signup"));
  ...
  useEffect(() => {
    ...
    if (checkoutNext) {
      setSignupHref(`${toPublicPageHref(locale, "/signup")}?next=${encodeURIComponent(checkoutNext)}`);
    }
  ...
  async function handleGoogleLogin() {
    ...
    // 검증해 이동한다(해당 라우트 자체는 next 처리를 바꾸지 않음). locale도 함께 실어
    // 보낸다 — /auth/confirm과 동일한 패턴("ko"만 한국어, 그 외 전부 일본어로 취급하는
    // 2택 enum)으로, OAuth 실패/취소 시 되돌아갈 로그인 페이지를 고르는 데만 쓰인다.
    const params = new URLSearchParams(window.location.search);
    const checkoutNext = buildCheckoutNext(params.get("next"), params.get("checkout"));
